(function () {
    ...
    HYPER.Graphics.Text = function (e) {
        this._ShapeType = "rect";
        this._ID = Math.random() * Math.random();

        this.cashed = false || e.cashed;
        this.image = document.createElement("canvas");
        this.ctx = this.image.getContext("2d");

        this.text = e.text || "Hello World";
        ...
        this.cash = function () {
            ...
            HYPER.Graphics.setStyle(this.ctx, this.style);
            this.image.width = this.ctx.measureText(this.text).width;
            this.image.height = this.style.fontSize;
        ...
        this._render = function (o) {
            if (this.visable == true) {
                ...
                if (this.degrees != 0) {
                    ...
                    if (this.cashed == true) {
                        o.ctx.drawImage(this.image, -this.rotation.x, -this.rotation.y, this.width, this.height)
                    } else {
                        if (this.style.fill == true) {
                            o.ctx.fillText(this.text, -this.rotation.x, -this.rotation.y);
                        }
                        if (this.style.stroke == true) {
                            o.ctx.strokeText(this.text, -this.rotation.x, -this.rotation.y);
                        }
                    }
                    ...
                    if (this.cashed == true) {
                        o.ctx.drawImage(this.image, this.x, this.y, this.width, this.height);
                    } else {
                        if (this.style.fill == true) {
                            o.ctx.fillText(this.text, this.x, this.y);
                        }
                        if (this.style.stroke == true) {
                            o.ctx.strokeText(this.text, this.x, this.y);
                        }
                    }
                }
